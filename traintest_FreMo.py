...
def predictModel(model, seq, dynamic_batch=True):
    ...
    for i in gen_batch(seq, min(args.batch_size, len(seq[0])), dynamic_batch=dynamic_batch):
        ...
        test_seq = np.copy(i[:, 0:args.seq_len, :, :, :])
        step_list = []
...
def modelInference(model, inputs):
    ...
    if args.seq_len + args.horizon > x_val[0].shape[0]:
    ...
    mae_val, rmse_val= get_metric(x_val[0:len_val, args.seq_len:args.horizon + args.seq_len, :, :, :], y_val[:, :, :, :, :], x_stats)
    # test
    y_test, len_test = predictModel(model, x_test)
    mae_test, rmse_test = get_metric(x_test[0:len_test, args.seq_len:args.horizon + args.seq_len, :, :, :], y_test[:, :, :, :, :], x_stats)
    return mae_val, rmse_val, mae_test, rmse_test


def traintest_model(dataset):
    model = get_model()    
    file_name = "{}_{}_his{}_pred{}_num_d{}_l{}_v{}".format(args.model_name, args.data_name, args.seq_len, args.horizon, args.d, args.latents, args.version)    
    ...
    for epoch in range(nb_epoch):  
        ...
        for j, x_batch in enumerate(gen_batch(dataset.get_data('train'), args.batch_size, dynamic_batch=True, shuffle=True)):
            optimizer.zero_grad()
            x = x_batch[:, 0:args.seq_len]            
            y = x_batch[:, args.seq_len:args.seq_len+args.horizon,:,:,:]
            x, y = prepare_x_y(x, y)            
            pred = model(x)
        ...
        print('=' * 80)
        print('Epoch {}: train_loss: {:.3f}; lr: {:.4f}; {:.1f}s'.format(
        epoch, train_loss, optimizer.param_groups[0]['lr'], (end_time - start_time)))            
...
parser.add_argument('--seq_len', default = 16, type=int, help = 'input length')
parser.add_argument('--horizon', default = 3, type=int, help = 'output length')
parser.add_argument('--latents', default = 64, type=int, help = 'number of inducing points')
parser.add_argument('--d', default = 64, type=int, help = 'number of hidden channels')
...
layers = int(np.log2(args.seq_len))
cpu_number = 1
...
########################################################### 
print('=' * 10)
print('| Model: {0} | Dataset: {1} | History: {2} | Horizon: {3}'.format(args.model_name, args.data_name, args.seq_len, args.horizon))
print("version: ", args.version)
print("hidden channels: ", args.d)
print("encoder layers: ", layers)
print("number of inducing points: ", args.latents)
# load data
print('=' * 10)
print("loading data...")
